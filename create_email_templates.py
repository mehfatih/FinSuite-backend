...
# Step 4: Create src/services/emailTemplates.ts
...
TARGET = SERVICES_DIR / "emailTemplates.ts"

print("=" * 70)
print("CREATE src/services/emailTemplates.ts")
print("=" * 70)

if TARGET.exists():
    print("[WARN] emailTemplates.ts already exists - will be OVERWRITTEN")
    print()
...
checks = [
    ("EmailLanguage type",          "export type EmailLanguage" in written),
    ("PlanWelcomeInput type",       "export type PlanWelcomeInput" in written),
    ("RenderedEmail type",          "export type RenderedEmail" in written),
    ("renderPlanWelcome export",    "export function renderPlanWelcome" in written),
    ("getEmailTemplate export",     "export function getEmailTemplate" in written),
    ("THEME TR red",                '"#E30A17"' in written),
    ("THEME AR green",              '"#006C35"' in written),
    ("STRINGS TR",                  "TR: {" in written and "Zyrix FinSuite" in written),
    ("STRINGS EN",                  "EN: {" in written),
    ("STRINGS AR",                  "AR: {" in written),
    ("FEATURE_LABELS TR",           "EFATURA_CREATE" in written),
    ("escapeHtml helper",           "function escapeHtml" in written),
    ("RTL handling",                'theme.dir === "rtl"' in written),
    ("HTML doctype",                "<!DOCTYPE html>" in written),
]
...
print("=" * 70)
print("[DONE] emailTemplates.ts created. Send output to Claude.")
print("=" * 70)