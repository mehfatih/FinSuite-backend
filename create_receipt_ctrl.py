# ============================================================
# Sprint 1 Phase 1A - Feature 2 / Step 2
# Create src/controllers/receiptScanController.ts
# 3 endpoints: scan, list, delete + convertToExpense (option)
# ============================================================

from pathlib import Path

ROOT = Path(r"D:\Zyrix Hub\zyrix-finsuite-backend")
TARGET = ROOT / "src" / "controllers" / "receiptScanController.ts"

print("=" * 70)
print("CREATE src/controllers/receiptScanController.ts")
print("=" * 70)
...
checks = [
    ("scanHandler export",        "export async function scanHandler" in written),
    ...
    ("deleteHandler export",      "export async function deleteHandler" in written),
    ("imports scanReceipt",       "from \"../services/receiptScanService\"" in written),
    ("imports prisma",            "from \"../config/database\"" in written),
    ("Zod scan schema",           "const scanSchema = z.object" in written),
    ("Zod list schema",           "const listSchema = z.object" in written),
    ("inserts PENDING row first", "PROCESSING" in written),
    ("auto-expense creation",     "prisma.expense.create" in written),
    ("CONVERTED status link",     "convertedExpenseId" in written),
    ("FAILED handling",           "FAILED" in written and "failureReason" in written),
]
