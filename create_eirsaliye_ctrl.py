# ============================================================
# Sprint 1 Phase 1A - Step 3
# Create src/controllers/eIrsaliyeController.ts
# 5 endpoints: create, list, get, update, queue
# ============================================================

from pathlib import Path

ROOT = Path(r"D:\Zyrix Hub\zyrix-finsuite-backend")
TARGET = ROOT / "src" / "controllers" / "eIrsaliyeController.ts"

print("=" * 70)
print("CREATE src/controllers/eIrsaliyeController.ts")
print("=" * 70)
...
checks = [
    ("createHandler export",     "export async function createHandler" in written),
    ("listHandler export",       "export async function listHandler" in written),
    ("getHandler export",        "export async function getHandler" in written),
    ("updateHandler export",     "export async function updateHandler" in written),
    ("queueHandler export",      "export async function queueHandler" in written),
    ("Zod create schema",        "const createSchema = z.object" in written),
    ("Zod update schema",        "const updateSchema = createSchema.partial" in written),
    ("Zod list schema",          "const listSchema = z.object" in written),
    ("imports XML service",      "from \"../services/eIrsaliyeXmlService\"" in written),
    ("imports prisma",           "from \"../config/database\"" in written),
    ("crypto.randomUUID",        "crypto.randomUUID" in written),
    ("DRAFT-only edit guard",    "Only DRAFT documents can be edited" in written),
    ("queue state machine",      'status !== "DRAFT" && status !== "READY_TO_SEND"' in written),
    ("XML preview in response",  "xmlPreview" in written),
]
