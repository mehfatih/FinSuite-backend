# ============================================================
# Sprint 1 Phase 1A - Feature 2 / Step 1
# Create src/services/receiptScanService.ts
# Uses Gemini Vision (gemini-1.5-flash) for OCR
# ============================================================

from pathlib import Path

ROOT = Path(r"D:\Zyrix Hub\zyrix-finsuite-backend")
TARGET = ROOT / "src" / "services" / "receiptScanService.ts"

print("=" * 70)
print("CREATE src/services/receiptScanService.ts")
print("=" * 70)
...
checks = [
    ("scanReceipt export",            "export async function scanReceipt" in written),
    ("ReceiptParseResult type",       "export type ReceiptParseResult" in written),
    ("imports GoogleGenerativeAI",    "import { GoogleGenerativeAI }" in written),
    ("imports env config",            "from \"../config/env\"" in written),
    ("uses gemini-1.5-flash model",   '"gemini-1.5-flash"' in written),
    ("strict JSON response",          'responseMimeType: "application/json"' in written),
    ("low temperature 0.1",           "temperature: 0.1" in written),
    ("stripCodeFences helper",        "function stripCodeFences" in written),
    ("safeNumber helper",             "function safeNumber" in written),
    ("safeString helper",             "function safeString" in written),
    ("normalizeIsoDate helper",       "function normalizeIsoDate" in written),
    ("supports data URLs",            "data:" in written),
    ("default currency TRY",          '"TRY"' in written),
]
